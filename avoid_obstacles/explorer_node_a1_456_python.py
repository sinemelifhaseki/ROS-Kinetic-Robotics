...
def laser_callback(data):
    ...
    leftDistance = 0
    rightDistance = 0
    for i in range (0,len(data.ranges)/2): #add up scan results
        rightDistance+=data.ranges[i]
    for i in range (len(data.ranges)/2,len(data.ranges)): #add up scan results
        leftDistance+=data.ranges[i]

    if data.ranges[len(data.ranges)/2] < 0.29 or (isnan(data.ranges[len(data.ranges)/2])and isnan(data.ranges[0])): #are we too close to obstacle?
        motor_command.linear.x = -1.5 #go backwards
        motor_command.angular.z = 0.3
        motor_command_publisher.publish(motor_command)
    
    if data.range_max > 4.2: #might be an exit?
        flag = True
    if flag == True:
        if data.ranges[-1] < 1:
            flag = False
        if data.ranges[-1] > 3.8:  #go left through door
            motor_command.linear.x = 1
            motor_command.angular.z = 1.4
            motor_command_publisher.publish(motor_command)
            if data.ranges[-1] < 3.88:
                motor_command.linear.x = 1
                motor_command.angular.z = 1.49
                motor_command_publisher.publish(motor_command)
        else:
            motor_command.linear.x = 1
            motor_command.angular.z = -1.38
            motor_command_publisher.publish(motor_command)
            if data.ranges[0] < 1.85:
                if data.ranges[-1]<3.52:
                    motor_command.linear.x = 1
                    motor_command.angular.z = 1.4
                    motor_command_publisher.publish(motor_command)
                else:
                    motor_command.linear.x = 1
                    motor_command.angular.z = -1.3
                    motor_command_publisher.publish(motor_command)    
            if data.ranges[len(data.ranges)/2] < 0.29 or (isnan(data.ranges[len(data.ranges)/2])and isnan(data.ranges[0])): #are we too close to obstacle?
                motor_command.linear.x = -1.75 #go backwards
                motor_command.angular.z = 0.3
                motor_command_publisher.publish(motor_command)
                motor_command.linear.x = 1.7 #go forward
                motor_command.angular.z = 0.1
                motor_command_publisher.publish(motor_command)
    else:
        motor_command.linear.x = 0.4 #go straight
        motor_command.angular.z = 0
        motor_command_publisher.publish(motor_command)
        if data.range_min > 0.4 and flag==False:
            motor_command.angular.z = 1.57 if leftDistance > rightDistance else -1.57
